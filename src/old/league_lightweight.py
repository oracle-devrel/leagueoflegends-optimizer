...
def get_top_players(region, queue):
	...
	for x in total_users_to_insert:
		...
		print('Obtained new summoner: {} in region {}, queue {}'.format(x['summonerName'], region, queue))
...
def match_list(players):
	...
	for x in players:
		current_summoner_puuid = get_summoner_information(x['summonerName'], x['request_region'])
