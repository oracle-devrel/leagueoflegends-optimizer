...
def match_list(db):
    ...
    # Query with specific columns to avoid index issues
    query = 'SELECT summonerId, summonerName, request_region FROM player_table'
    ...
    for x in all_summoners:
        ...
        request_region = x[2].lower()
        print('[{}][INFO] SUMMONER {} REGION {}'.format(time.strftime("%Y-%m-%d %H:%M"), current_summoner, request_region))
        # Use summoner ID to get PUUID (summoner names are no longer reliable)
        current_summoner_puuid = get_summoner_information_by_id(
            summoner_id, request_region.lower())
        if current_summoner_puuid is None:
            print('[{}][SKIP] Could not get PUUID for {}'.format(time.strftime("%Y-%m-%d %H:%M"), current_summoner))
            continue
